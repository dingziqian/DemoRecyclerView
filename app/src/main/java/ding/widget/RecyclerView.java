...
import com.ding.demorecyclerview.R;
...
import ding.widget.RecyclerView.ItemAnimator.ItemHolderInfo;

import static android.support.annotation.RestrictTo.Scope.GROUP_ID;
import static ding.widget.AdapterHelper.Callback;
import static ding.widget.AdapterHelper.UpdateOp;
...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild {
     ...
     * <p>
     * 如果你没有提供一个pool的话，recyclerView会自动创建一个
     * RecyclerView automatically creates a pool for itself if you don't provide one.
     *
     */
    public static class RecycledViewPool {
        // 根据 viewType 保存的被废弃的 ViewHolder 集合，以便下次使用 viewType --> ArrayList<ViewHolder> 的映射
        private SparseArray<ArrayList<ViewHolder>> mScrap =
                new SparseArray<ArrayList<ViewHolder>>();
        // 各种 viewType 对应的最大缓存数  viewType --> maxNum 的映射
        ...
        public void setMaxRecycledViews(int viewType, int max) {
            ...
            // 如果viewType类型的list的size大于制定的最大数字的话，会先从列表的末尾开始丢弃超出的部分
            if (scrapHeap != null) {
        ...
        /**
         * 获取缓存中的ViewHolder
         * 将mScrap中的最后一项移除并返回viewType对应的List的末尾项
         * @param viewType
         * @return
         */
        ...
        public void putRecycledView(ViewHolder scrap) {
            ...
            if (mMaxScrap.get(viewType) <= scrapHeap.size()) { // 超过了最大值
                return;
            }
        ...
        /**
         * Detaches 老的adapter  attach 新的adapter
    ...
    /**
     * 管理那些已经废弃了的或者从RecyclerView中分离的item view用于复用。
     ...
     *
     * Recycler典型的用法：为adapter数据集获取view，展示特定位置(position)或者特定itemID的数据。
     * 如果被重用的view是失效的（dirty），则需要adapter重新绑定view.
     * 如果是有效的(clean)， 则可以直接复用。
     * 对于有效(clean)的view， 如果不主动调用isLayoutRequested()来请求layout，
     * 则LayoutManager重新定位时就不需要重新测量大小。
     ...
     *
     * 这个类中有两个缓存：Scrap和Recycle，这两个缓存有什么作用呢？
     * 首先Scrap缓存是指里面缓存的View是接下来需要用到的，即里面的绑定的数据无需更改，
     * 可以直接拿来用的，是一个轻量级的缓存集合；
     * 而Recycle的缓存的View为里面的数据需要重新绑定，即需要通过Adapter重新绑定数据
     * Detach的View放在Scrap缓存中，Remove掉的View放在Recycle缓存中；
     *
     * 在什么样的场景中使用Detach呢？
     * 主要是在我们的代码执行结束之前，我们需要反复去将View移除并且马上又要添加进去时，
     * 选择Datach方式，比如：当我们对View进行重新排序的时候，可以选择Detach，
     * 因为屏幕上显示的就是这些position对应的View，我们并不需要重新去绑定数据，
     * 这明显可以提高效率。使用Detach方式可以通过函数detachAndScrapView()实现。
     *
     * 而使用Remove的方式，是当View不在屏幕中有任何显示的时候，你需要将它Remove掉，
     * 以备后面循环利用。可以通过函数removeAndRecycleView()实现
     *
     *
     */
    public final class Recycler {
        ...
        ArrayList<ViewHolder> mChangedScrap = null; // 表示数据已经改变的VewHolder列表
        // ViewHolder缓存列表，其大小由mViewCacheMax决定，默认DEFAULT_CACHE_SIZE为2，可动态设置。
        final ArrayList<ViewHolder> mCachedViews = new ArrayList<ViewHolder>();
        // mAttachedScrap不可修改类型的列表
        private final List<ViewHolder>
                mUnmodifiableAttachedScrap = Collections.unmodifiableList(mAttachedScrap);

        private int mRequestedCacheMax = DEFAULT_CACHE_SIZE;
        // mCacheView的最大值， 即ViewHolder缓存的最大值， 默认为2.
        int mViewCacheMax = DEFAULT_CACHE_SIZE;
        // ViewHolder缓存池，在有限的mCachedViews中如果存不下ViewHolder时，就会把ViewHolder存入RecyclerViewPool中
        ...
        /**
         * 清空当前依附于RecyclerView的ViewHolder， 将缓存的ViewHolder移动到RecycledPool中。
        ...
        /**
         * 返回当前依附RecyclerView的ViewHolder列表（mAttachedScrap）,
         * 需要注意的是这里返回的是不可修改的的列表mUnmodifiableAttachedScrap。
        ...
        /**
         *  该方法是getViewForPosition方法的辅助方法，
         *  用于检验给出的ViewHolder对于某个位置是否可用。如果匹配则为true, 否则为false。
        ...
        /**
         * 该方法的作用是将一个View（ViewHolder）绑定到给定位置。该View可以是通过getViewForPosition(int)得到的，
         * 也可以是通过 Adapter.onCreateViewHolder(ViewGroup, int)创建的。
         * 通常情况下， 一个LayoutManager应该通过getViewForPosition获取View，并交由 RecyclerView处理缓存。
         * 但对于要自己处理复用逻辑的LayoutManager,该方法是LayoutManager自己处理复用逻辑的辅助方法。
         * 另外需要注意的是： getViewForPosition获取到的View是已经绑定过position的。
         * 所以如果你不想要绑定另一个position的话，并不需要调用该方法。
         *
        ...
        /**
         * RecyclerView在pre-layout状态下会提供人造的位置范围（item count），即当前可以绘制的item总数，
         * 当调用getViewForPosition或者bindViewToPosition时会自动将该范围内的position映射对应adapter的位置position。
         * 通常情况下， LayoutManager不需要关心这些。然而，在某些情况下，比如你的LayoutManager调用自定义组件，
         * 需要传入一个真实的adapter中的位置position时，
         * 就可以调用该方法将pre-layout状态下的位置转化为adapter（post layout）状态下单位置。
         * 需要注意的是：如果传入的位置所对应的ViewHolder已被删除，则返回－1。
         * 并且传入的position的位置应该在［0，itemCount］之间 。
         * 如果在post layout状态调用该方法，则返回结果 ＝ 传入的position
         *
        ...
        /**
         * 获取某个位置的View。
         * 该方法应由LayoutManager调用，实现获取View展示adapter中的数据的目的。
         * 如果ViewHolder的viewType正确的话，Recycler会重用共享池中的scrap或者detached view，
         * 即ViewHolder。如果adapter没有说明给定位置的数据有变化，
         * Recycler则会尝试返回一个之前初始化的scrap（ViewHolder），不需要重新绑定数据。
        ...
        /**
         * 回收一个缓存View，并将这个View从list当中移除。这些View被添加到缓存池当中，所以这个方法没有在check
         ...
         * <p>
         * 此规则的一个小例外是当视图没有动画引用，但是transient状态为true(因为是在ItemAnimator之外创建的动画)
         * 这种情况下adapter会选择回收它，从RecyclerView的角度来看，
         * 由于Adaptor想要这样做，这些View仍然是可回收的。
        ...
        /**
         * 分发ViewHolder被recycle的消息，mRecyclerListener、mAdapter 回调
         * @param holder
         */
    ...
    /**
     * ViewCacheExtension是一个帮助类，给开发者提供了一个可以由他们自己控制View缓存的这么一个类
     * ViewCacheExtension is a helper class to provide an additional layer of view caching that can
     * be controlled by the developer.
     * 当调用Recycler的getViewForPosition(int position)方法的时候，
     * Recycler会先去检查attached scrap和一级缓存 来找寻这个View，如果都找不到匹配的View的话，
     * Recycler会调用ViewCacheExtension的getViewForPositionAndType方法检查，
     * 如果还是没有，再去检查RecycledViewPool。
     *
     * <p>
    ...
    public abstract static class ViewCacheExtension {
        ...
        /**
         * 返回一个可以绑定到adapter对应的position的View
         ...
         * <p>
         * 这个方法不应该创造一个新的View，相反，它被期望返回一个已经创建好的View，这个View应该可以根据给定的Type和position被再利用
         * 如果这个View被标记为ignored，应该先调用LayoutManager#stopIgnoringView(View)
    ...
    /**
     * ViewHolder描述RecylerView中某个位置的itemView和元数据信息
     *
     * A ViewHolder describes an item view and metadata about its place within the RecyclerView.
     *
     * 属于Adapter的一部分，其实现类通常用于保存findViewById的结果
     * <p>{@link Adapter} implementations should subclass ViewHolder and add fields for caching
     * potentially expensive {@link View#findViewById(int)} results.</p>
     *
     * 这个类当中的LayoutParams是归属于LayoutManager，ViewHolders是归属于adapter的，
     *
     * Adapters应该可以自由使用自定义的ViewHolder来存储使绑定视图内容更容易的数据
     * 实现者应该假设每一个itemView都有一个对ViewHolder的强引用，
     * 并且RecyclerView的实例可能会对屏幕外的item view有一个强引用来进行高速缓存
     *
    ...
    public static abstract class ViewHolder {
        ...
        /**
         * 表示ViewHolder已绑定某个位置的数据，mPosition, mItemId and mItemViewType都是可用的。
         ...
         * mPosition 和 mItemId 是不被信任的，可能不再匹配item的view type
        ...
        /**
         * 这个ViewHolder是从scrap返回，这意味着我们期待着为itemView调用addView方法，
         * 当我们从scrap中获取这个viewHolder的时候，这个ViewHolder会一直保存在Scrap的list当中，
         * 直到布局从末尾滑过，它如果没有被添加到recyclerView当中，就会被recyclerView回收
        ...
        /**
         * ViewHolder完全由LayoutManager管理，不能回收， 重用， 移除 直到LayoutManager被替换
         ...
         * 它和FLAG_INVALID不同，因为FLAG_INVALID是当类型没有匹配的时候就设置了，当然了，
         * FLAG_ADAPTER_POSITION_UNKNOWN是当adapter通知到达的时候就设置了，
         * 而FLAG_INVALID是在布局从新计算之前就被设置了
        ...
        /**
         * 当ViewHolder的位置发生变化时候，在ItemAnimator当中使用
